...
const StepOne = ({ nextStep }) => {
  ...
  return (
    <div className="step-one">
      ...
      {/* Форма торта */}
      <div className="param__group">
        <label htmlFor="shape" className='label__title'>Форма торта</label>
        <select
          id="shape"
          name="form"  // Изменил name на "form" для соответствия структуре хука
          value={cakeData.form || ''}
          onChange={handleChange}
          required
        >
      ...
      <div className="param__group param__counter">
        ...
        <div className="param__counter-wrapper">
          ...
          <div className="param__counter-info">
            <p>Вес торта ~ <span>{cakeData.weight || '2.0'} кг</span> из расчёта 200 гр на человека</p>
          </div>
        </div>
      </div>
    </div>
